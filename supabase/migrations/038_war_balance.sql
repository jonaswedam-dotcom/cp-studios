-- Migration 038: Balance — loot economy. Idempotent. Run by hand in the Supabase SQL editor
-- in numerical order (CLAUDE.md §5), AFTER 036 (it depends on war_stack_strength from 036).
-- SUPERSEDES the 036 definition of public.war_tick().
--
-- ONE change vs 036: the captured-province loot coefficient (COIN_PER_STRENGTH) 5 → 15.
-- Conquering an enemy province previously paid floor(0.8 * def_raw * 5) coins — roughly 25×
-- less than the army value spent to win the fight, so rational players turtled instead of
-- fighting. Raising the rate to 15 makes a won assault far less coin-negative without turning
-- conquest into a money printer. Mirrors src/war/spoils.js COIN_PER_STRENGTH (parity.test.js).
--
-- Everything else in war_tick() below is byte-for-byte identical to 036 (the loot line at the
-- "capture" branch is the only edit). war_stack_strength(), the building type check, the
-- pg_cron schedule, and all RLS/grants are unchanged and remain as established by 036/024.
