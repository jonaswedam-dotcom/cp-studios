-- Migration 048: Income rebalance. Idempotent. Run by hand in the Supabase SQL editor
-- in numerical order (CLAUDE.md §5), AFTER 038 (it supersedes 038's war_tick()).
--
-- Soldiers cost 100 and armies run into the hundreds, so the old passive income
-- (banks×50 + provinces×10/hr) couldn't keep up. This 2.5× the rates and stretches the
-- offline-accrual window so a player who's away from their computer comes back to a
-- meaningful sum:
--   * bank income:     50  -> 125 coins/level/hour   (mirrors src/war/buildings.js)
--   * province income: 10  -> 25  coins/hour          (mirrors src/war/buildings.js)
--   * offline cap:     rate×10h -> rate×16h (vault keeps filling for 16h, not 10)
--
-- SUPERSEDES the 038 definition of public.war_tick(). Everything below is byte-for-byte
-- identical to 038 EXCEPT the income block (section 2): the two rate literals and the
-- cap multiplier. war_stack_strength(), the combat branches, the pg_cron schedule, and
-- all RLS/grants are unchanged and remain as established by 036/038/024.
...
begin
    ...
    if not found or dest.owner_id is null then
      ...
      if a_eff > d_eff then
        ...
        perform public.war_log_event(mv.player_id, 'attack_failed', mv.to_region,
          jsonb_build_object('neutral', true));
      end if; -- attack failed, units lost
      ...
      if def_shield is not null and def_shield > now() then
        ...
        def_raw := dest.soldier * 1 + dest.tank * 5 + dest.jet * 3 + dest.warship * 20;
        ...
        if a_eff > d_eff then
          ...
          surv_ratio := (a_eff - d_eff) / a_eff;
          loot := floor(0.8 * def_raw * 15); -- lootFraction(full kill)=0.8 × defStrength × COIN_PER_STRENGTH(15)
          if loot > 0 then
  ...
  -- 2) Accrue capped income: banks×125 + provinces×25 (coins/hr), cap = rate×16h.
  with base as (
    ...
    select user_id, vault, last_income_at,
           (banklv*125 + provinces*25) as rate,
           floor((banklv*125 + provinces*25) * greatest(0, extract(epoch from (now()-last_income_at))/3600.0)) as accrued
    from base
  )
  update public.war_players p set
    vault = least(p.vault + c.accrued, greatest(p.vault, c.rate*16))::int,
    last_income_at = case
