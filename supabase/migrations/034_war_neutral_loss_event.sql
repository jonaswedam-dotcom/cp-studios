-- Migration 034: log neutral-attack losses (previously silent). Tick body copied from 027,
-- only the neutral attack-failed branch changed: a failed assault on a NEUTRAL province used to
-- log NOTHING (the troops just vanished and it looked like a bug). It now logs an 'attack_failed'
-- event carrying meta.neutral=true so the player gets feedback in the feed + toast.
--
-- Idempotent (create or replace). Like all migrations here, it must be run by hand in the
-- Supabase SQL editor in numerical order (CLAUDE.md §5). It SUPERSEDES the 027 definition of
-- public.war_tick() — everything else is byte-for-byte identical to 027.
