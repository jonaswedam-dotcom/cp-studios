-- Migration 036: Ports + OP warships. Idempotent. Run by hand in the Supabase SQL editor
-- in numerical order (CLAUDE.md §5). SUPERSEDES the 034 definition of public.war_tick().
--
-- Two changes vs 034, both about warships:
--   1) A new 'port' building type. Ports are a coastal-only building (enforced client-side,
--      same trust model as unit purchases — see CLAUDE.md §4). You must own a port to build
--      warships, and they deploy to the port.
--   2) Warship strength 2 → 20 ("really OP, because they cost a lot"). The coefficient lives
--      in TWO SQL spots — war_stack_strength() (attacking stacks) and the tick's def_raw
--      defender formula — and in src/war/units.js. parity.test.js guards all three.
-- Everything else in war_tick() is byte-for-byte identical to 034.

-- 1) Allow the new 'port' building type (the 021 inline check listed only five types).
alter table public.war_buildings drop constraint if exists war_buildings_type_check;
alter table public.war_buildings add constraint war_buildings_type_check
  check (type in ('bunker','antiair','factory','lab','bank','port'));

-- 2) Stack strength with the OP warship coefficient. Mirrors src/war/combat.js stackStrength.
...
language sql immutable as $$
  select coalesce((s->>'soldier')::numeric,0)*1 + coalesce((s->>'tank')::numeric,0)*5
       + coalesce((s->>'jet')::numeric,0)*3 + coalesce((s->>'warship')::numeric,0)*20;
$$;

-- 3) Live tick (copy of 034; only the def_raw warship coefficient changed 2 → 20).
